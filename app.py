...
def chat():
    ...
    if not HERMES_KEY:
        return jsonify({
            "error": "Hermes API server key is not configured"
        }), 500
    ...
    # -----------------------------------------------------
    # Standard OpenAI-compatible model name.
    #
    # Provider selection is configured inside Hermes:
    # model.provider = custom:groq
    #
    # Do NOT use:
    # custom:groq:openai/gpt-oss-120b
    #
    # in this REST payload unless the installed Hermes
    # REST implementation is separately verified to support it.
    # -----------------------------------------------------
    payload = {
        "model": MODEL_NAME,
        "messages": [
    ...
    try:
        ...
        # -------------------------------------------------
        # Do not turn an upstream Hermes error into HTTP 200.
        # -------------------------------------------------
        ...
        return jsonify({
            "error": "Hermes Agent request timed out."
        }), 504
